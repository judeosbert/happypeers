...
export default async function removeEmployeeEp(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method !== "POST") {
...
async function removeEmployee(req: RemoveEmployeeReq) {
  ...
  if (!res || res.status != 200) {
    throw Error("Error Deleting the user");
  }
}
